...
A few weeks ago, I decided to start putting together some of the cybersecurity questions I was asked over the past few years, so that I can come back and review the questions and answers.

By no means I am suggesting this post is complete and comprehensive with correct answers, since my attempt is to answer the questions in such a way that not just communicate the security basics but also points reader to the right direction whenever possible.

I would appreciate if you read and comment on any part of this documentation.


## SSH and TLS. Name a few differences between the two.

They both runs on top of TCP/IP. But the two are different in many ways. 

Port

The well known port for SSH is 22. TLS protocol itself does not specify any port, so it is up to the application to specify a port, e.g. HTTPS typically uses port 443.
...
## Name a few network services and respective well known ports

This question is rather pentester related than GRC related. 
...
## Why publickey method is considered better than password in SSH authentication
...
In a nutshell, DH provides a solution to a very essential cryptographic requirement: To allow two parties who have never met to agree on a shared secret key by communication in such a way that the secret can't be seen by observing the communication. There is an important distinction in here: The two are not sharing an existing scret key during the communication, they are creating a secret key together.

[diffie hellman video]: https://youtu.be/YEBfamv-_do?list=PL87386AD236727A1B

## forward secrecy

forward secrecy (FS), is a feature of specific key agreement protocols that gives assurances your session keys will not be compromised even if the private key of the server is compromised. 

key exchange, hash, MAC, asymmetric and symmetric enyption all play a part in the effort of providing FS capabilities. 
...
There are just too many LOL. The key words there are "went public". Needless to say, there are many more breaches that gone unnoticed or have not gone public just yet. For starters, visit [List of data breaches](https://en.wikipedia.org/wiki/List_of_data_breaches). Verizon data breach investigation report is also worth reading. Be advised that there are many reports published by government and private sectors that discuss some breaches and cyber attacks in greater detail. 


## Describe "The Cyber(intrusion) Kill Chain"

A framework/model developed by Lockheed-Martin. It intends to reveal the phases of a cyber attack: 7 steps from early reconnaissance to the goal of data exfiltration. The framework is used by organizations to define phases of a typical cyber attack.
...
## Common concerns with bastion host (jump server)
...
## What is Federation and how it works

To be added. [https://aws.amazon.com/blogs/security/aws-federated-authentication-with-active-directory-federation-services-ad-fs/](https://aws.amazon.com/blogs/security/aws-federated-authentication-with-active-directory-federation-services-ad-fs/) 
...
## The different between scripting language and compiled language
...
## Common 2FA methods? How does Google Authenticator work?
...
## Describe an example of defense-in-depth
...
To be added. [https://www.gremwell.com/ssh-mitm-public-key-authentication](https://www.gremwell.com/ssh-mitm-public-key-authentication) 
...
AWS whitepapers pertaining to security should be read first. Visit the website at [https://aws.amazon.com/whitepapers/#security](https://aws.amazon.com/whitepapers/#security) for latest publications. 
...
KMS: master keys. Only used to encrypt and decrypt up to 4KB data, use it to generate, encrypt and decrypt the data keys. It is so called envelope encryption [https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html) 
...
OWASP top 10 vulnerabilities for web application security. The complete report can be found [here](https://www.owasp.org/index.php/Category:OWASP_Top_Ten_Project) on OWASP website. The OWASP Top 10 is a powerful awareness document for web application security. It represents a broad consensus about the most critical security risks to web applications. 
...
A SQL injection attack consists of insertion or "injection" of a SQL query via the input data from the client to the application. SQL injection attacks are a type of injection attack, in which SQL commands are injected into data-plane input in order to effect the execution of predefined SQL commands. 


## CSRF 

Cross-Site Request Forgery is an attack that forces an end user to execute unwanted actions on a web application in which they're currently authenticated. CSRF attacks specifically target state-changing requests, not theft of data, since the attacker has no way to see the response to the forged request. With a little help of social engineering (such as sending a link via email or chat), an attacker may trick the users of a web application into executing actions of the attacker's choosing. If the victim is a normal user, a successful CSRF attack can force the user to perform state changing requests like transferring funds, changing their email address, and so forth. If the victim is an administrative account, CSRF can compromise the entire web application. 

How does it work? Check out on [OWASP](https://www.owasp.org/index.php/Cross-Site_Request_Forgery_(CSRF)) where two scenarios were given: one using HTTP GET, the other one using HTTP POST and javascript.
...
There are two common categories: stored and reflected. There is a third, much less well-known type of XSS attack called DOM Based XSS.
...
As far as I am concerned, there is a rather generic requirement in many domains: The need to uniquely identify an individual entity with no ambiguity. Many different approaches have been invented (and re-invented over and over again) since to fulfil it in their respective domain. To name a few, social security number, credit number, phone number, GUID, URI, IP address etc. ARN is just another approach that Amazon uses within the AWS world.
...
[https://medium.com/stax-blog/aws-account-structure-strategies-for-enterprises-a6bae76be487](https://medium.com/stax-blog/aws-account-structure-strategies-for-enterprises-a6bae76be487) 


## AWS Service Control Policies (SCP) 

SCPs are similar to IAM permission policies and use almost the exact same syntax. However, an SCP never grants permissions. Instead, think of an SCP as a filter that enables you to restrict what service and actions can be accessed by users and roles in the accounts that you attach the SCP to. 
...
Amazon EBS Public Snapshots (Free!) - check if Amazon EBS) volume snapshots and alerts you if any snapshots are marked as public.
...
SCA is a relatively new industry term for a set of tools that provides users visibility into their open source inventory. SCA tools come in different forms, offering a range of capabilities from those focused on licensing compliance only to others encompassing both security and license management. 
...
- Why is SCA becoming an integral part of tooling
