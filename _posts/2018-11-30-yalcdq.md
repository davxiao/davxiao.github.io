...
A few weeks ago, I decided to put together some of the cybersecurity questions I was asked over the past few years, so that I can come back and review the questions and answers from time to time. This post is my first deliverable.

By no means I am suggesting this post is complete and comprehensive, as my attempt is to provide some answers to the questions in such that it is not just communicating the security basics but also providing additional references. I believe all the questions here have been asked and answered many times on the internet, so I gave credits to the original posts and materials by providing references wherever applicable.
...
SFTP stands for Secure FTP. SFTP by itself does not provide authentication and security; it expects the underlying protocol to secure this. SFTP is most often used as subsystem of SSH *(credit: wikipedia)*
...
### Why publickey method is better than password in SSH
...
Elliptic Curve Diffie–Hellman (ECDH) is a variation of DH which provides better security. Latest OpenSSH has started using ECDH. See more details at [weakdh.org](https://weakdh.org/){:target="_blank"} and [wikipedia](https://en.wikipedia.org/wiki/Logjam_(computer_security))
...
- Privacy concerns. A device may be passively tracked by its WiFi MAC address, because a stable MAC address was being used to scan for WiFi networks. Randomizing the MAC address was indended to address the concern, but articles such as [A Study of MAC Address Randomization in Mobile Devices and When it Fails](https://arxiv.org/abs/1703.02874){:target="_blank"} argue that MAC address randomization policies are neither universally implemented nor effective at eliminating such concerns
...
Block cipher mode: ECB and CBC. [Wikipedia](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation){:target="_blank"} explains why ECB and CBC are not advisable cipher mode.
...
- Push-based 2FA
   - Like Apple’s Trusted Devices method, can send a prompt to one of your devices during login. This prompt will indicate that someone (possibly you) is trying to log in, and an estimated location for the login attempt. You can then approve or deny the attempt.
   - It’s not standardized, so you can’t choose from a variety of authenticator apps, and can’t consolidate all your push-based credentials in a single app. Also, it requires a working data connection on your phone, while Authenticator apps don’t require any connection

- Universal Second Factor (U2F)
   - Relatively new, typically using small USB, NFC or Bluetooth devices called “security keys.” To set it up on a site, you register your U2F device. On subsequent logins, the site will prompt you to connect your device and tap it to allow the login.
...
Security questions have a similar purpose, but are, ultimately, less secure than two-factor authentication because they’re “what you know” factors.
...
AWS security blogs about GDPR
<https://aws.amazon.com/blogs/security/tag/gdpr/>
...
Here are a couple of other things that you should know about PCI DSS:
...
**CHD**, **SAD**, **CDE**, **Network segmentation** in regard to compliance requirements: 
...
See more detail at [PCI DSS data storage](https://www.pcisecuritystandards.org/pdfs/pci_fs_data_storage.pdf)

Cardhold data environment(CDE) is the systems that processes, stores, transmits CHD or SAD. It also includes any component that directly connects to or supports this network.

Since it is critical to effectively manage the scope of CDE in reality, Network segmentation is a best practice to significantly reduce the scope of the PCI environment. As reference, PCI DSS [mandated the scoping concepts](https://www.pcisecuritystandards.org/documents/Guidance-PCI-DSS-Scoping-and-Segmentation_v1.pdf) as follows:
...
In AWS PCI DSS context, there is standardized architecture available, which provides a compliance checklist which provides a number of accelerators by mapping PCI-DSS requirements to AWS services and functionalities.
...
AWS Security Workshops follow the NIST Cybersecurity Framework 5 core functions: Identify, Protect, Detect, Respond, Recover.
